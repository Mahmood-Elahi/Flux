...
int main(int argc, char** argv) {
    if (argc != 5) {
        std::cerr << "Usage: softmax_cuda_cli <input.bin> <output.bin> <rows> "
                     "<row-width>\n";
        return 2;
    }
    ...
    try {
        ...
        const std::size_t byte_count =
            static_cast<std::size_t>(checked_byte_count(element_count));
        check_cuda(
            cudaStreamCreateWithFlags(&stream, cudaStreamNonBlocking),
            "cudaStreamCreateWithFlags");
        check_cuda(
            cudaMalloc(reinterpret_cast<void**>(&device_input), byte_count),
            "cudaMalloc input");
        check_cuda(
            cudaMalloc(reinterpret_cast<void**>(&device_output), byte_count),
            "cudaMalloc output");
        check_cuda(
            cudaMemcpyAsync(
                device_input,
        ...
        std::cerr << "softmax_cuda_cli: " << error.what() << '\n';
