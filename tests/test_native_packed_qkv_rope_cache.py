...
def test_out_contract_returns_and_overwrites_supplied_tensor() -> None:
    ...
    expected_length = length.clone()
    expected = packed_qkv_rope_cache_native(
        packed, cos, sin, expected_keys, expected_values, expected_length
    )
    output = torch.full((1, 9, 1, 64), 83.0, device="cuda")
    assert packed_qkv_rope_cache_native_out(
        packed, cos, sin, keys, values, length, output
    ) is output
