"""Compare standalone C++ residual RMSNorm with the Flux PyTorch oracle."""
...
def _read_outputs(
    ...
    if len(values) != 2 * count:
        raise RuntimeError(
            f"C++ output has {len(values)} values; expected {2 * count}"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "executable", type=Path, help="path to residual_rmsnorm_cli.exe"
    )
    ...
    print("C++ residual RMSNorm matches the PyTorch oracle within FP32 tolerances.")
    return 0
