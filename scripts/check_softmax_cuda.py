"""Compare standalone CUDA softmax with the Flux PyTorch oracle."""
...
def _make_cases() -> list[tuple[str, torch.Tensor]]:
    ...
    for case_index, shape in enumerate(RANDOM_SHAPES):
        generator = torch.Generator().manual_seed(3000 + case_index)
...
def main() -> int:
    ...
    parser.add_argument(
        "executable", type=Path, help="path to softmax_cuda_cli.exe"
    )
    ...
    cases = _make_cases()
    with tempfile.TemporaryDirectory(prefix="flux-softmax-cuda-") as temp_directory:
        temp_path = Path(temp_directory)
        for case_index, (case_name, input_tensor) in enumerate(cases):
            ...
            if not torch.isfinite(actual).all():
                raise AssertionError(f"{case_name}: CUDA output is not finite")
            if not torch.all(actual >= 0):
                raise AssertionError(f"{case_name}: CUDA output is negative")
    ...
    print(f"maximum absolute error: {overall_absolute:.9g}")
    print(f"maximum relative error: {overall_relative:.9g}")
    print(f"maximum row-sum error: {overall_row_sum_error:.9g}")
    print("CUDA softmax matches the Flux PyTorch oracle within FP32 tolerances.")
    return 0
